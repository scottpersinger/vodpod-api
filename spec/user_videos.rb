...
module API
  describe '/users/<user>/videos' do
    behaves_like :rack_test
    behaves_like :api

    should 'list videos for a user' do
      json('/users/aphyr/videos', :sort => :created_at) do |j|
        j['total'].should > 10
        j = j['results']
    ...
    should 'list videos with a tag' do
      json('/users/aphyr/videos', :tag => 'neotokyo', :include => 'tags') do |j|
        j['results'].each do |v|
    ...
    should 'not list videos with any nonexistent tags' do
      json('/users/aphyr/videos', :tag => 'foobeezee', :include => 'tags')['total'].should == 0
    end

    should 'sort by most popular' do
      json('/users/aphyr/videos', :sort => 'popular') do |j|
    ...
    should 'sort by ranking' do
      json('/users/aphyr/videos', :sort => 'ranking', :include => 'ranking', :limit => 100) do |j|
      ...
      json('/users/aphyr/videos', :sort => 'ranking', :order => 'asc', :include => 'ranking') do |j|
        j['results'].each_with_index do |v, i|
          if succ = j['results'][i + 1] and v['ranking'] and succ['ranking']
            v['ranking'].should <= succ['ranking']
          end
    ...
    should 'display a particular video' do
      json('/users/aphyr/videos/1767013', :include => 'tags,title,key') do |j|
    ...
    should 'not display nonexistent videos' do
      json_error('/users/aphyr/videos/343')['message'].should =~ /not been collected/
    end
   
    should 'filter video information' do
      json('/users/aphyr/videos/278382', :include => 'key,title,tags') do |j|
    ...
    should 'have a user and collection' do
      json('/users/aphyr/videos/278382', :include => 'user,collection') do |j|
    ...
    should 'have tags' do
      json('/users/aphyr/videos/278382', :include => 'tags') do |j|
    ...
    should 'have an embed tag' do
      json('/users/aphyr/videos/1869137') do |j|
        j['embed'].should =~ /http:/
    ...
    should 'have comments' do
      json('/users/aphyr/videos/1869137', :include => 'comments') do |j|
        j['comments'].any? { |c| c['text'] =~ /6000/ }.should.be.true
      end
    end

    should 'have collection comments' do
      json('/users/aphyr/videos/1869137', :include => 'collection_comments') do |j|
